function [] = plot_TE_Pooled_Sh_newChan_combinedFeatures(TEpath,clusterParams,params,FIT_params,reach_bins,plot_params)

animal_colors = distinguishable_colors(numel(params.animals));
MI_info = load('G:\CurrBiol_revision\data\processed_data\pooled\MI\MI_24-Jun-2021_LFPearlylate_1_LFPday_0_Spikesday_0_MatchTrials_0_POOLED.mat');

for n_lfp = 1:length(params.lfpFeatures)

    tmpFiles = ls([TEpath 'TE*']);
    ...
    for animal = 1:size(tmpFiles,1)
        ...
        for fidx = 1:length(params.reachFeatures)
            ...
            for pairs = bothSig
                tmp_info = squeeze(TEout{animal}.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_M1receiver(pairs,:,:));
                tmp_infoSh = squeeze(TEout{animal}.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_M1receiverSh(pairs,:,:,:));
                ...
                tmp_info = squeeze(TEout{animal}.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_DLSreceiver(pairs,:,:));
                tmp_infoSh = squeeze(TEout{animal}.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_DLSreceiverSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                %                 tmp_info(~tmp_sig) = 0;
            ...
            pairIdx = 1;            
            for pairs = bothSig
                tmp_info = squeeze(TEout{animal}.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_M1receiver(pairs,:,:));
                tmp_infoSh = squeeze(TEout{animal}.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_M1receiverSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                %                 tmp_info(~tmp_sig) = 0;
                late_info_M1receiver(pairIdx,:,:) = tmp_info;
                tmp_info = squeeze(TEout{animal}.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_DLSreceiver(pairs,:,:));
                tmp_infoSh = squeeze(TEout{animal}.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).TE_negd_DLSreceiverSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                %                 tmp_info(~tmp_sig) = 0;
    ...
    %% PLOT MEAN INFO OVER TIME AND DELAYS (IMAGESC)
        ...
        figure('units','normalized','outerposition',[0 0 1 1]);
            ...
            clearvars g
                ...
                g(3,1).axe_property('YLim',[0 0.03]);
                ...
                g(3,2).axe_property('YLim',[0 0.03]);
                g(3,2).set_title(['late']);
                g.draw();
                if plot_params.save == 1
                    print([plot_params.save_path '\TE_imagesc.eps'],'-painters','-depsc');
                end
        ...
        %% HISTOGRAM
            ...
            figure;
                ...
                if plot_params.save == 1
                    print([plot_params.save_path '\TE_histogram.eps'],'-painters','-depsc');
                end          
