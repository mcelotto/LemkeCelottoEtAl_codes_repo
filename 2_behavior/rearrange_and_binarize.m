...
for animal = 1:numel(params.animals)
    ...
    %%% Lowpass/Highpass
    if bin_params.doLowpass || bin_params.doHighpass
        for day = 1:numel(animal_reachFeatures)
            disp(['Bandpassing animal # ', num2str(animal), ' day # ', num2str(day), ' | ' num2str(bin_params.frequencyCutoff), 'Hz']);
            for areaIdx = 1:length(params.areas)
                for lfpIdx = 1:length(params.lfpFeatures)
                    for chan = 1:size(animal_neuralActivity.LFP{day}.(params.lfpFeatures{lfpIdx}).(params.areas{areaIdx}),1)
                        if bin_params.doLowpass
                            animal_neuralActivity.LFP{day}.(params.lfpFeatures{lfpIdx}).(params.areas{areaIdx})(chan,:,:) = lowpass(squeeze(animal_neuralActivity.LFP{day}.(params.lfpFeatures{lfpIdx}).(params.areas{areaIdx})(chan,:,:)),bin_params.frequencyCutoff,1000/bin_params.oldBin);
                        else
                            animal_neuralActivity.LFP{day}.(params.lfpFeatures{lfpIdx}).(params.areas{areaIdx})(chan,:,:) = highpass(squeeze(animal_neuralActivity.LFP{day}.(params.lfpFeatures{lfpIdx}).(params.areas{areaIdx})(chan,:,:)),bin_params.frequencyCutoff,1000/bin_params.oldBin);
                        end
                    end
                end
            end
