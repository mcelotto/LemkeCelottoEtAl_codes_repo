function [FIT_biased, varargout] = FIT(Z, hX, hY, Y, varargin)

%%% *function [FIT_biased, varargout] = FIT(Z, hX, hY, Y, varargin)*
%%%
%%% ### Description
%%% This function computes the feature-specific information transmission FIT, either naive or bias-corrected, given input data.
%%%
%%% ### Inputs:
%%% - *S*: must be an array of *nDimsZ X nTrials * elements representing the discrete value of the stimulus presented in each trial.
...
%%% | opts.bin_method_Z                    | specifies the binning method for the Z signals                                                                                                                                                                                                               | `'none'` (no binning)<br>`'eqpop'` (evenly populated binning)<br>`'eqspace'`(evenly spaced binning)<br>`'ceqspace'`(centered evenly spaced binning)<br>`'gseqspace'`(gaussian evenly spaced binning)<br> See the documentation of [[binr|binr]] function for more details   | `'eqpop'` |
%%% | opts.n_bins                          | number of bins to be used to reduce the dimensionality of the response                                                                                                                                                                                  | int > 1                                                                                                                                                                                                                                            | 3         |
...
assert( length(Z(1,:)) == length(hX(1,:)) || ...
     length(Z(1,:)) == length(hY(1,:)) || ...
     length(Z(1,:)) == length(hY(1,:)), ...
...
% assert(length(Z(1,:)) == 1,...
%     "Stimulus vector should be given as a column vector");
% assert(length(hX(1,:)) == length(Z(:,1)),...
%     "Number of columns of hX, hX(1,:), should be of size nTrials");
% assert(length(hY(1,:)) == length(Z(:,1)),...
%     "Number of columns of hY, hY(1,:), should be of size nTrials");
% assert(length(Y(1,:)) == length(Z(:,1)),...
%     "Number of columns of Y, Y(1,:), should be of size nTrials");
% assert(length(hY(:,1)) == length(Y(:,1)),...
%     "Number of rows of hY and Y should be equal to nDimsY");

% check for nans
if any(isnan(Z))
    error("Z contains NaNs. Aborting.")
end
if any(isnan(hX))
...
if nargin == 4
    ...
    if ~isfield(opts,'bin_method_Z')
        opts.bin_method_Z = 'eqpop';
    else
        assert(strcmp(opts.bin_method_Z,'none') ||...
            strcmp(opts.bin_method_Z,'eqpop') ||...
            strcmp(opts.bin_method_Z, 'eqspace') ||...
            strcmp(opts.bin_method_Z, 'ceqspace') ||...
            strcmp(opts.bin_method_Z, 'gseqspace'),...
            ['opts.bin_method_Z argument can be only ''eqpop'', ''eqspace'', ''ceqspace'' or ''gseqspace''. ',...
            'Specified value ''%s'''], opts.bin_method_Z);
    end
    ...
    if ~isfield(opts,'n_bins')
        ...
        assert(opts.n_bins > 1,...
            'opts.n_bins should be > 1');
    end   
...
for i=2:numel(split_factors)
    ...
    assert(max_n_draws <= nchoosek(n_trials,n_split_trials), ['Number of ',...
        ...
        opts.bin_method, sf, floor(nchoosek(n_trials,n_split_trials)*sf));
end

if min(Z) <= 0
    Z = Z - min(Z) + 1; % avoid negative value for the probabilityDist function
end
% bin neural responses if required
if ~strcmp(opts.bin_method_Z, 'none')
    Z_b = binr(Z, opts.n_bins, opts.bin_method) + 1;
else
    Z_b = Z + 1;
end

if ~strcmp(opts.bin_method_X, 'none')
    hX_b = binr(hX, opts.n_bins, opts.bin_method) + 1; % The '+ 1' is added because zeroes in the array would cause an error in the probabilityDist function
else
    hX_b = hX + 1;
end

if ~strcmp(opts.bin_method_Y, 'none')
    hY_b = binr(hY, opts.n_bins, opts.bin_method) + 1;
    Y_b = binr(Y, opts.n_bins, opts.bin_method) + 1;
else
    hY_b = hY + 1;
    Y_b = Y + 1;
end
...
for s = 1:n_splits
    ...
    for d = 1:max_n_draws
        ...
        FIT_tmp(d) = calculate_feature_specific_information_transmission(Z_b',hX_b,hY_b,Y_b,n_trials,n_split_trials(s));
