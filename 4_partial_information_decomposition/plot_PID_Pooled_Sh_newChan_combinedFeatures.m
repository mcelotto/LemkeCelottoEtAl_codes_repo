function [] = plot_PID_Pooled_Sh_newChan_combinedFeatures(PIDpath,clusterParams,params,reach_bins,plot_params)
...
for n_lfp = 1:length(params.lfpFeatures)
    ...
    figure('units','normalized','outerposition',[0 0 1 1]);
        subplot(2,2,1); hold on;
            ...
            imagesc(squeeze(mean(tmp_infoe,1))',[0 0.014])
        ...
        subplot(2,2,2); hold on;
            ...
            imagesc(squeeze(mean(tmp_infol,1))',[0 0.022])
            plot([0.5 39.5],[25.5 25.5],'color','r')
            plot([0.5 39.5],[26.5 26.5],'color','r')
            plot([26 26],[.5 51.5],'color','k')
            colorbar;
        ...
        clearvars g
            ...
            g(2,1).axe_property('YLim',[0 0.014]);    
            ...
            g(2,2).axe_property('YLim',[0 0.018]);    
            g(2,2).set_title(['late']);        
            g.draw();
        if plot_params.save == 1
            print([plot_params.save_path '\PID_info.eps'],'-painters','-depsc');
        end
    ...
    figure('units','normalized','outerposition',[0 0 1 1]);
        ...
        subplot(2,2,3); hold on;
            ...
            xlabel('time from pellet touch (ms)')
        if plot_params.save == 1
            print([plot_params.save_path '\PID_info_diff.eps'],'-painters','-depsc');
        end
    ...
    subplot(2,2,4);
        ...
        grid off;
    if plot_params.save == 1
        print([plot_params.save_path '\PID_cdfs.eps'],'-painters','-depsc');
    end
    ...
    if plot_params.save == 1
        print([plot_params.save_path '\PID_histogram.eps'],'-painters','-depsc');
    end    
    ...
    figure;
        ...
        title([num2str(p1)])
    if plot_params.save == 1
        print([plot_params.save_path '\PID_by_animal.eps'],'-painters','-depsc');
    end          
