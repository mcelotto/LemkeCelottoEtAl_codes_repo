function [] = plot_PID_Pooled_Sh_newChan_combinedFeatures_successMatch(PIDpath,clusterParams,params,reach_bins,plot_params)
...
for n_lfp = 1:length(params.lfpFeatures)
    ...
    figure('units','normalized','outerposition',[0 0 1 1]);
        subplot(2,2,1); hold on;
            ...
            imagesc(squeeze(mean(tmp_infoe,1))',[0 0.01])
            ...
            ylim([.5 51.5])
            yticks(1:2:51)
            yticklabels([-250:20:-10 10:20:250])
        ...
        subplot(2,2,2); hold on;
            ...
            imagesc(squeeze(mean(tmp_infol,1))',[0 0.03])
            plot([0.5 39.5],[25.5 25.5],'color','r')
            plot([0.5 39.5],[26.5 26.5],'color','r')
            plot([26 26],[.5 51.5],'color','k')
            colorbar;
        ...
        clearvars g
            ...
            g(2,1).axe_property('YLim',[0 0.01]);    
            ...
            g(2,2).axe_property('YLim',[0 0.03]);    
            g(2,2).set_title(['late']);        
            g.draw();
        if plot_params.save == 1
            print([plot_params.save_path '\PID_info_successMatch.eps'],'-painters','-depsc');
        end

    figure('units','normalized','outerposition',[0 0 1 1]);
        subplot(2,2,1); hold on;
            imagesc(squeeze(mean(tmp_infol,1))'-squeeze(mean(tmp_infoe,1))',[-.015 .03])
        ...
        subplot(2,2,3); hold on;
            ...
            xlabel('time from pellet touch (ms)')
        if plot_params.save == 1
            print([plot_params.save_path '\PID_info_diff_successMatch.eps'],'-painters','-depsc');
        end
    ...
    figure;
    subplot(2,2,1); hold on;
        histogram(max_early_M1toDLS,[0:0.01:.2],'Normalization','Probability','DisplayStyle','Stairs');    
        histogram(max_early_DLStoM1,[0:0.01:.2],'Normalization','Probability','DisplayStyle','Stairs');
    ...
    subplot(2,2,3);
        ...
        grid off;
    subplot(2,2,2); hold on;
        histogram(max_late_M1toDLS,[0:0.01:.2],'Normalization','Probability','DisplayStyle','Stairs');    
        histogram(max_late_DLStoM1,[0:0.01:.2],'Normalization','Probability','DisplayStyle','Stairs');
    ...
    subplot(2,2,4);
        ...
        grid off;
    if plot_params.save == 1
        print([plot_params.save_path '\PID_cdfs_successMatch.eps'],'-painters','-depsc');
    end
    ...
    if plot_params.save == 1
        print([plot_params.save_path '\PID_histogram_successMatch.eps'],'-painters','-depsc');
    end    
    ...
    figure;
        ...
        title([num2str(p1)])
    if plot_params.save == 1
        print([plot_params.save_path '\PID_by_animal_successMatch.eps'],'-painters','-depsc');
    end          
