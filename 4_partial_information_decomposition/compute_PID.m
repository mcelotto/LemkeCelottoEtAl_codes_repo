function [PIDout] = compute_PID(LFP, spikes, reachFeatures, LFP_early_late, reachFeatures_early_late, params, PID_params, tMin, binSize, animals_to_run)
    ...
    for animal = animals_to_run
                ...
                for aidx = 1:length(params.areas)
                    for early_late = 1:2
                        ...
                        for chan = 1:size(MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(params.reachFeatures{fidx}).info,1)
                            ...
                            sigTime_start = find(rebinned_binTimes==PID_params.MIsigtiming(1));
                            sigTime_end = find(rebinned_binTimes==PID_params.MIsigtiming(2));
                    ...
                    parfor pairIdx = 1:size(tmp_pairs,1)
                        ...
                        for timeStep = 1:length(tmp_timeStep)
                            ...
                            for delay = 1:length(tmp_delays)
                                ...
                                X1 = tmp_M1_LFP(tmp_timeStep(timeStep),:);
                                X2 = tmp_DLS_LFP(tmp_timeStep(timeStep)+tmp_delays(delay),:);
                                ...
                                pair_shared(timeStep,delay) = I.shared;
                                pair_unique_m1(timeStep,delay) = I.uniqueX1;
                                pair_unique_dls(timeStep,delay) = I.uniqueX2;
                                pair_complementary(timeStep,delay) = I.complementary;
                                ...
                                if PID_params.doShuff
                                    for shidx = 1:PID_params.nShuff
                                        ...
                                        pair_shared_Sh(timeStep,delay,shidx) = I.shared;
                                        pair_unique_m1_Sh(timeStep,delay,shidx) = I.uniqueX1;
                                        pair_unique_dls_Sh(timeStep,delay,shidx) = I.uniqueX2;
                                        pair_complementary_Sh(timeStep,delay,shidx) = I.complementary;
        ...
        if PID_params.savePID
            save([PID_params.save_path 'PID_' params.animals{animal} '_' date ...
                ...
                '_maxVel.mat'],'-v7.3','PIDout','params','PID_params')
        end
...
%                 if PID_params.doShuff
%                     shuffIdxs = zeros(size(spikes{animal}{day}.(params.areas{1}),3),PID_params.nShuff);
%                     for shidx = 1:PID_params.nShuff
%                         shuffIdxs(:,shidx) = randperm(length(nonNaNtrials));
%                     end
%                 end
