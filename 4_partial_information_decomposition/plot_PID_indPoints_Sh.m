function [] = plot_PID_indPoints_Sh(PIDpath,clusterParams,params,reach_bins,plot_params)
...
for n_lfp = 1:length(params.lfpFeatures)
    for fidx = 1:length(params.reachFeatures)
        ...
        for animal = 1:size(tmpFiles,1)-1
...
%                 tmp_info(~tmp_sig) = 0;
        ...
        if fidx==1
            ylim_info = [0 0.01];
            ylim_info_diff = [-.008 .008];
        elseif fidx==2
            ylim_info = [0 0.02];
            ylim_info_diff = [-.02 .02];
        end
        ...
        %%% PLOT MEAN INFO OVER TIME AND DELAYS (IMAGESC)
            ...
            figure('units','normalized','outerposition',[0 0 1 1]);
                ...
                if plot_params.save == 1
                    print([plot_params.save_path '\PID_indPoints_sharedInfo_' params.reachFeatures{fidx} '.eps'],'-painters','-depsc');
                end
...
%                     colormap(redblue)
                ...
                subplot(2,2,3); hold on;
                    ...
                    xlabel('time from pellet touch (ms)')
                if plot_params.save == 1
                    print([plot_params.save_path '\PID_indPoints_sharedInfo_difference_' params.reachFeatures{fidx} '.eps'],'-painters','-depsc');
                end
        ...
        %%% PLOT DELAYS
            ...
            figure;
                ...
                subplot(1,2,2); hold on;
                    ...
                    plot([5.5 5.5],[-.25 .25],'color','r','LineStyle','--')
                    xticks([1:1:10])     
                    xticklabels({'-250 to -200','-200 to -150','-150 to -100','-100 to -50','-50 to 0','0 to 50','50 to 100','100 to 150','150 to 200','200 to 250'})
                    ylim([-.25 .25])                
                if plot_params.save == 1
                    print([plot_params.save_path '\PID_indPoints_sharedInfo_delays_' params.reachFeatures{fidx} '.eps'],'-painters','-depsc');
                end          
            ...
            if plot_params.save == 1
                print([plot_params.save_path '\PID_indPoints_sharedInfo_delays_by_animal_' params.reachFeatures{fidx} '.eps'],'-painters','-depsc');
            end    
        ...
        %%% SUMMARY STATS
            ...
            figure('units','normalized','outerposition',[0 0 1 1]);
                ...
                g(1,1).stat_boxplot();
                [h p1] = ttest(max_early_neg,max_early_neg_Sh)
                [h p2] = ttest(max_early_pos,max_early_pos_Sh)
                [h p3] = ttest(max_early_neg,max_early_pos)
                g(1,1).set_title(['M1 -> DLS p=' num2str(p2) ' | DLS -> M1 p=' num2str(p1) ' | Real LvsN p=' num2str(p3)]);
                ...
                g(1,2).stat_boxplot();
                [h p1] = ttest(max_late_neg,max_late_neg_Sh)
                [h p2] = ttest(max_late_pos,max_late_pos_Sh)
                [h p3] = ttest(max_late_neg,max_late_pos)
                g(1,2).set_title(['M1 -> DLS p=' num2str(p2) ' | DLS -> M1 p=' num2str(p1) ' | Real LvsN p=' num2str(p3)]);
                ...
                if plot_params.save == 1
                    print([plot_params.save_path '\PID_indPoints_sharedInfo_summary_stats_' params.reachFeatures{fidx} '.eps'],'-painters','-depsc');
                end    
