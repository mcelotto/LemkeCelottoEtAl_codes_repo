function [] = plot_PID_Pooled_Sh_success_fail(PIDpath,clusterParams,params,reach_bins,plot_params)
...
for n_lfp = 1:length(params.lfpFeatures)
    for fidx = 1:length(params.reachFeatures)
        ...
        early_info_all_failure = [];
        early_info_Orig_all_failure = [];
        early_info_Sh_all_failure = [];
        early_pairs_per_animal_failure = 0;
        early_info_all_success = [];
        early_info_Orig_all_success = [];
        early_info_Sh_all_success = [];
        early_pairs_per_animal_success = 0;

        late_info_all_failure = [];
        late_info_Orig_all_failure = [];
        late_info_Sh_all_failure = [];
        late_pairs_per_animal_failure = 0;
        late_info_all_success = [];
        late_info_Orig_all_success = [];
        late_info_Sh_all_success = [];
        late_pairs_per_animal_success = 0;
        ...
        for animal = 1:size(tmpFiles,1)
            ...
            load([PIDpath tmpFiles(animal,:)],'PIDout')
            
            %%% EARLY SUCCESS
            early_info_success = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            early_info_Orig_success = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            early_info_Sh_success = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            for pairs = 1:size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1)
                tmp_info = squeeze(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared(pairs,:,:));
                tmp_infoSh = squeeze(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.sharedSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                early_info_Orig_success(pairs,:,:) = tmp_info;
                early_info_Sh_success(pairs,:,:) = squeeze(mean(tmp_infoSh,3));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                tmp_info(~tmp_sig) = 0;
                early_info_success(pairs,:,:) = tmp_info;
            end
            early_info_all_success = cat(1,early_info_all_success,early_info_success);
            early_info_Orig_all_success = cat(1,early_info_Orig_all_success,early_info_Orig_success);
            early_info_Sh_all_success = cat(1,early_info_Sh_all_success,early_info_Sh_success);
            early_pairs_per_animal_success = [early_pairs_per_animal_success size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1)];

            %%% EARLY FAILURE
            early_info_failure = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            early_info_Orig_failure = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            early_info_Sh_failure = zeros(size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            for pairs = 1:size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1)
                tmp_info = squeeze(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared(pairs,:,:));
                tmp_infoSh = squeeze(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.sharedSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                early_info_Orig_failure(pairs,:,:) = tmp_info;
                early_info_Sh_failure(pairs,:,:) = squeeze(mean(tmp_infoSh,3));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                tmp_info(~tmp_sig) = 0;
                early_info_failure(pairs,:,:) = tmp_info;
            end
            early_info_all_failure = cat(1,early_info_all_failure,early_info_failure);
            early_info_Orig_all_failure = cat(1,early_info_Orig_all_failure,early_info_Orig_failure);
            early_info_Sh_all_failure = cat(1,early_info_Sh_all_failure,early_info_Sh_failure);
            early_pairs_per_animal_failure = [early_pairs_per_animal_failure size(PIDout.LFP_early_late{1}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1)];

            %%% LATE SUCCESS
            late_info_success = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            late_info_Orig_success = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            late_info_Sh_success = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1),39,51);
            for pairs = 1:size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1)
                tmp_info = squeeze(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared(pairs,:,:));
                tmp_infoSh = squeeze(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.sharedSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                late_info_Orig_success(pairs,:,:) = tmp_info;
                late_info_Sh_success(pairs,:,:) = squeeze(mean(tmp_infoSh,3));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                tmp_info(~tmp_sig) = 0;
                late_info_success(pairs,:,:) = tmp_info;
            end
            late_info_all_success = cat(1,late_info_all_success,late_info_success);
            late_info_Orig_all_success = cat(1,late_info_Orig_all_success,late_info_Orig_success);
            late_info_Sh_all_success = cat(1,late_info_Sh_all_success,late_info_Sh_success);
            late_pairs_per_animal_success = [late_pairs_per_animal_success size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).success.shared,1)];

            %%% LATE FAILURE
            late_info_failure = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            late_info_Orig_failure = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            late_info_Sh_failure = zeros(size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1),39,51);
            for pairs = 1:size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1)
                tmp_info = squeeze(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared(pairs,:,:));
                tmp_infoSh = squeeze(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.sharedSh(pairs,:,:,:));
                tmp_sig = clusterStat_v2(tmp_info,tmp_infoSh,clusterParams(1),clusterParams(2));
                late_info_Orig_failure(pairs,:,:) = tmp_info;
                late_info_Sh_failure(pairs,:,:) = squeeze(mean(tmp_infoSh,3));
                tmp_info = tmp_info-squeeze(mean(tmp_infoSh,3));
                tmp_info(~tmp_sig) = 0;
                late_info_failure(pairs,:,:) = tmp_info;
            end
            late_info_all_failure = cat(1,late_info_all_failure,late_info_failure);
            late_info_Orig_all_failure = cat(1,late_info_Orig_all_failure,late_info_Orig_failure);
            late_info_Sh_all_failure = cat(1,late_info_Sh_all_failure,late_info_Sh_failure);
            late_pairs_per_animal_failure = [late_pairs_per_animal_failure size(PIDout.LFP_early_late{2}.(params.lfpFeatures{n_lfp}).(params.reachFeatures{fidx}).failure.shared,1)];
        ...
        %% PLOT MEAN INFO OVER TIME AND DELAYS (IMAGESC) SUCCESS

            figure('units','normalized','outerposition',[0 0 1 1]);

                subplot(2,2,1); hold on;
                    imagesc(squeeze(mean(early_info_all_success,1))')
                    ...
                    title(num2str(size(early_info_all_success,1)));
                subplot(2,2,2); hold on;
                    imagesc(squeeze(mean(late_info_all_success,1))')
                    plot([0.5 39.5],[25.5 25.5],'color','r')
                    plot([0.5 39.5],[26.5 26.5],'color','r')
                    plot([26 26],[.5 51.5],'color','k')
                    colorbar;
                    ...
                    yticklabels(-250:20:250)
                    ylabel('DLS time delay (ms)')
                    title(num2str(size(late_info_all_success,1)));                    
                clearvars g
                    x = [1:39];
                    y = [mean(early_info_all_success(:,:,27:51),3); mean(early_info_all_success(:,:,1:25),3)];
                    c = [ones(1,size(mean(early_info_all_success(:,:,27:51),3),1)) 2*ones(1,size(mean(early_info_all_success(:,:,1:25),3),1))];
                    ...
                    y = [mean(late_info_all_success(:,:,27:51),3); mean(late_info_all_success(:,:,1:25),3)];
                    c = [ones(1,size(mean(late_info_all_success(:,:,27:51),3),1)) 2*ones(1,size(mean(late_info_all_success(:,:,1:25),3),1))];
            ...
            figure('units','normalized','outerposition',[0 0 1 1]);
                subplot(2,2,1); hold on;
                    imagesc(squeeze(mean(late_info_all_success,1))'-squeeze(mean(early_info_all_success,1))',ylim_info_diff)
                    colormap(redblue)
                    plot([0.5 39.5],[25.5 25.5],'color','r')
                    plot([0.5 39.5],[26.5 26.5],'color','r')
                    plot([26 26],[.5 51.5],'color','k')
                ...
                subplot(2,2,3); hold on;
                    plot(mean(mean(late_info_all_success(:,:,27:51),3),1)-mean(mean(early_info_all_success(:,:,27:51),3),1),'color',[.5 1 .25],'LineWidth',2);
                    plot(mean(mean(late_info_all_success(:,:,1:25),3),1)-mean(mean(early_info_all_success(:,:,1:25),3),1),'color',[0 .25 0],'LineWidth',2);
                    xlim([6 36])
                    ylim(ylim_info_diff)
                    xticks([6 16 26 36])
                    xticklabels([-1 -.5 0 .5])
        ...
        %% PLOT MEAN INFO OVER TIME AND DELAYS (IMAGESC) FAILURE
            
            figure('units','normalized','outerposition',[0 0 1 1]);

                subplot(2,2,1); hold on;
                    imagesc(squeeze(mean(early_info_all_failure,1))')
                    plot([0.5 39.5],[25.5 25.5],'color','r')
                    plot([0.5 39.5],[26.5 26.5],'color','r')
                    plot([26 26],[.5 51.5],'color','k')
                    colorbar;
                    ...
                    yticklabels(-250:20:250)
                    ylabel('DLS time delay (ms)')
                    title(num2str(size(early_info_all_failure,1)));
                subplot(2,2,2); hold on;
                    imagesc(squeeze(mean(late_info_all_failure,1))')
                    plot([0.5 39.5],[25.5 25.5],'color','r')
                    plot([0.5 39.5],[26.5 26.5],'color','r')
                    plot([26 26],[.5 51.5],'color','k')
                    colorbar;
                    ...
                    yticklabels(-250:20:250)
                    ylabel('DLS time delay (ms)')
                    title(num2str(size(late_info_all_failure,1)));                    
                clearvars g
                    x = [1:39];
                    y = [mean(early_info_all_failure(:,:,27:51),3); mean(early_info_all_failure(:,:,1:25),3)];
                    c = [ones(1,size(mean(early_info_all_failure(:,:,27:51),3),1)) 2*ones(1,size(mean(early_info_all_failure(:,:,1:25),3),1))];
                    g(2,1)=gramm('x',x,'y',y,'color',c);
                    g(2,1).stat_summary('type','sem');
                    g(2,1).axe_property('XLim',[6 36]);
                    g(2,1).axe_property('YLim');    
                    g(2,1).set_title(['early']);        
                    y = [mean(late_info_all_failure(:,:,27:51),3); mean(late_info_all_failure(:,:,1:25),3)];
                    c = [ones(1,size(mean(late_info_all_failure(:,:,27:51),3),1)) 2*ones(1,size(mean(late_info_all_failure(:,:,1:25),3),1))];
                    g(2,2)=gramm('x',x,'y',y,'color',c);
                    g(2,2).stat_summary('type','sem');
                    g(2,2).axe_property('XLim',[6 36]);
                    g(2,2).axe_property('YLim',ylim_info);    
            ...
            figure('units','normalized','outerposition',[0 0 1 1]);
                subplot(2,2,1); hold on;
                    imagesc(squeeze(mean(late_info_all_failure,1))'-squeeze(mean(early_info_all_failure,1))',ylim_info_diff)
                    colormap(redblue)
                    plot([0.5 39.5],[25.5 25.5],'color','r')
                    plot([0.5 39.5],[26.5 26.5],'color','r')
                    plot([26 26],[.5 51.5],'color','k')
                    ...
                    ylabel('DLS time delay (ms)')
                subplot(2,2,3); hold on;
                    plot(mean(mean(late_info_all_failure(:,:,27:51),3),1)-mean(mean(early_info_all_failure(:,:,27:51),3),1),'color',[.5 1 .25],'LineWidth',2);
                    plot(mean(mean(late_info_all_failure(:,:,1:25),3),1)-mean(mean(early_info_all_failure(:,:,1:25),3),1),'color',[0 .25 0],'LineWidth',2);
                    xlim([6 36])
                    ylim(ylim_info_diff)
                    xticks([6 16 26 36])
                    xticklabels([-1 -.5 0 .5])
        ...
        %% HISTOGRAM SUCCESS
            ...
            max_early_DLStoM1 = [];
            for pair = 1:size(early_info_all_success,1)
                max_early_DLStoM1 = [max_early_DLStoM1 max(squeeze(early_info_all_success(pair,reach_bins,1:25)),[],'all')];
            end
            max_early_M1toDLS = [];
            for pair = 1:size(early_info_all_success,1)
                max_early_M1toDLS = [max_early_M1toDLS max(squeeze(early_info_all_success(pair,reach_bins,27:51)),[],'all')];
            end
            max_late_DLStoM1 = [];
            for pair = 1:size(late_info_all_success,1)
                max_late_DLStoM1 = [max_late_DLStoM1 max(squeeze(late_info_all_success(pair,reach_bins,1:25)),[],'all')];
            end
            max_late_M1toDLS = [];
            for pair = 1:size(late_info_all_success,1)
                max_late_M1toDLS = [max_late_M1toDLS max(squeeze(late_info_all_success(pair,reach_bins,27:51)),[],'all')];
            end
...
%% PLOT DELAYS SUCCESS
            ...
            early_peakDelay = cell(1,length(early_pairs_per_animal_success)-1);
            late_peakDelay = cell(1,length(early_pairs_per_animal_success)-1);
            
            for animal = 1:length(early_pairs_per_animal_success)-1
                                
                tmp_peakDelay = [];
                for pair=1+sum(early_pairs_per_animal_success(1:animal)):sum(early_pairs_per_animal_success(1:animal+1))
                    [~,time_i] = max(squeeze(max(early_info_all_success(pair,:,:),[],3)));                    
                    if ismember(time_i,reach_bins)
                        [~, i] = max(squeeze(max(early_info_all_success(pair,reach_bins,:),[],2)));
                ...
                tmp_peakDelay = [];
                for pair = 1+sum(late_pairs_per_animal_success(1:animal)):sum(late_pairs_per_animal_success(1:animal+1))
                    [~,time_i] = max(squeeze(max(late_info_all_success(pair,:,:),[],3)));                    
                    if ismember(time_i,reach_bins)
                        [~, i] = max(squeeze(max(late_info_all_success(pair,reach_bins,:),[],2)));                        
                        tmp_peakDelay = [tmp_peakDelay i];
                    else
                        tmp_peakDelay = [tmp_peakDelay nan];
                    end
...
%% PLOT DELAYS FAILURE
            ...
            early_peakDelay = cell(1,length(early_pairs_per_animal_failure)-1);
            late_peakDelay = cell(1,length(early_pairs_per_animal_failure)-1);
            
            for animal = 1:length(early_pairs_per_animal_failure)-1
                                
                tmp_peakDelay = [];
                for pair=1+sum(early_pairs_per_animal_failure(1:animal)):sum(early_pairs_per_animal_failure(1:animal+1))
                    [~,time_i] = max(squeeze(max(early_info_all_failure(pair,:,:),[],3)));                    
                    if ismember(time_i,reach_bins)
                        [~, i] = max(squeeze(max(early_info_all_failure(pair,reach_bins,:),[],2)));
                        tmp_peakDelay = [tmp_peakDelay i];
                    else
                        tmp_peakDelay = [tmp_peakDelay nan];
                    end
                ...
                tmp_peakDelay = [];
                for pair = 1+sum(late_pairs_per_animal_failure(1:animal)):sum(late_pairs_per_animal_failure(1:animal+1))
                    [~,time_i] = max(squeeze(max(late_info_all_failure(pair,:,:),[],3)));                    
                    if ismember(time_i,reach_bins)
                        [~, i] = max(squeeze(max(late_info_all_failure(pair,reach_bins,:),[],2)));                        
                        tmp_peakDelay = [tmp_peakDelay i];
                    else
                        tmp_peakDelay = [tmp_peakDelay nan];
                    end
