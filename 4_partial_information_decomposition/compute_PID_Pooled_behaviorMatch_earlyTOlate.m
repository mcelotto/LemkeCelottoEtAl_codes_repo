function [PIDout] = compute_PID_Pooled_behaviorMatch_earlyTOlate(MI_chanSelection_LFP,LFP_early_late, reachFeatures_early_late, reachFeatures_early_late_noBinNoTrialMatch, params, PID_params, tMin, bin_params, animals_to_run,features_to_run,features_to_match,save_params)
    ...
    for animal = animals_to_run
        ...
        for fmatch = features_to_match
            ...
            for fidx = features_to_run
                ...
                all_early_vals_match = reachFeatures_early_late_noBinNoTrialMatch{animal}{1}.(params.reachFeatures{fmatch});
                ...
                for i = 1:length(reachFeatures_early_late_noBinNoTrialMatch{animal}{2}.(params.reachFeatures{fmatch}))
                    tmp_late_val_match = reachFeatures_early_late_noBinNoTrialMatch{animal}{2}.(params.reachFeatures{fmatch})(i);
                    tmp_late_val = reachFeatures_early_late_noBinNoTrialMatch{animal}{2}.(params.reachFeatures{fidx})(i);
                    [val, idx] = min(abs(all_early_vals_match-tmp_late_val_match));
                    if val<dist_thresh
                        if ~isnan(tmp_late_val) && ~isnan(all_early_vals(idx))
