function [PIDout] = compute_instPID_Pooled(LFP_early_late, spikes_early_late, reachFeatures, reachFeatures_early_late, params, PID_params, tMin, binSize, animals_to_run,features_to_run,features_to_run_static,kin_times)
% Time always referenced to putative receiving region

% Fixed kinematics time points

PID_params.save_path = [PID_params.save_path, '/instPID/100Shuff_recRef/fixedKin_', num2str(kin_times(1)),'_',num2str(kin_times(end)), '/'];
if ~exist(PID_params.save_path,'dir')
    mkdir(PID_params.save_path)
end
...
if PID_params.doLFPearlylate==1
    ...
    %MI_instant = load(PID_params.path_to_instMI);
    ...
    for animal = animals_to_run
        ...
        sig_channels = cell(length(params.lfpFeatures),length(params.instReachFeatures),length(params.areas),2);
        for n_lfp = 1:length(params.lfpFeatures)
            for fidx = 1:numel(features_to_run_static)
                for aidx = 1:length(params.areas)
                    for early_late = 1:2
                        sig_chan = [];
                        for chan = 1:size(MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(features_to_run_static{fidx}).info,1)
                            infQuant = MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(features_to_run_static{fidx}).info(chan,:);
                            infQuantSh = MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(features_to_run_static{fidx}).infoSh(chan,:,:);
                            tmp_sig = clusterStat_v2(infQuant,infQuantSh,PID_params.MIclusterparams(1),PID_params.MIclusterparams(2));
                            infQuant(~tmp_sig) = 0;
                            [~,i] = max(infQuant);
                            sigTime_start = find(MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(features_to_run_static{fidx}).binTimes==PID_params.MIsigtiming(1));
                            sigTime_end = find(MI_info.MIout{animal}.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx}).(features_to_run_static{fidx}).binTimes==PID_params.MIsigtiming(2));
        ...
        for fidx = 1:numel(features_to_run)
            for early_late = 1:2
                for n_lfp = 1:length(params.lfpFeatures)
                    
                    nonNaNtrials = find(~isnan(sum(reachFeatures_early_late{animal}{early_late}.instant.(features_to_run{fidx}),1)));
                    ...
                    tmp_delays = -25:25; % submitted -25:25
                    ...
                    parfor pairIdx = 1:size(tmp_pairs,1)
                        ...
                        disp(['Computing: ' params.animals{animal} ' | M1 channel ' num2str(m1_chan) ' & DLS channel ' num2str(dls_chan) ' | ' num2str(early_late) ' of 2 (early/late) | ' features_to_run{fidx} ' | neural-reach lag ',num2str(kin_times)])
                        ...
                        for timeStep = 1:length(tmp_timeStep)
                            ...
                            for delay = tmp_delays
                                ...
                                if sTime > 26 & eTime < 200
                                    ...
                                    Y = reachFeatures_early_late{animal}{early_late}.instant.(features_to_run{fidx})(kin_times,nonNaNtrials);
    ...
    %                                         Y = repmat(Y,size(X1,1),1);
                    ...
                    PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).shared = tmp_shared;
                    PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).unique_m1 = tmp_unique_m1;
                    PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).unique_dls = tmp_unique_dls;
                    PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).complementary = tmp_complementary;
                    
                    if PID_params.doShuff
                        PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).sharedSh = tmp_shared_Sh;
                        PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).unique_m1Sh = tmp_unique_m1_Sh;
                        PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).unique_dlsSh = tmp_unique_dls_Sh;
                        PIDout.LFP_early_late{early_late}.(params.lfpFeatures{n_lfp}).(features_to_run{fidx}).complementarySh = tmp_complementary_Sh;
                    end
        ...
        % save results
        if PID_params.savePID==1
            save([PID_params.save_path '/PID_recRef_inst_' params.animals{animal} '_' date ...
                '_100shuff_LFPearlylate_' num2str(PID_params.doLFPearlylate) ...
...
% if PID_params.doSpikes==1
%     for animal = animals_to_run
%         
%         % determine channels with significant mutual information
%         sig_units = cell(length(params.instReachFeatures),length(params.areas),length(MI_info.MIout{animal}.spikesDay));           
%         for fidx = 1:length(params.instReachFeatures)
%             for aidx = 1:length(params.areas)
%                 for day = 1:length(MI_info.MIout{animal}.spikesDay)
%                     sig_unit = [];
%                     for unit = 1:size(MI_info.MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.instReachFeatures{fidx}).info,1)
%                         infQuant = MI_info.MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.instReachFeatures{fidx}).info(unit,:);
%                         infQuantSh = MI_info.MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.instReachFeatures{fidx}).infoSh(unit,:,:);
%                         tmp_sig = clusterStat_v2(infQuant,infQuantSh,PID_params.MIclusterparams(1),PID_params.MIclusterparams(2));
%                         infQuant(~tmp_sig) = 0;
%                         [~,i] = max(infQuant);
%                         sigTime_start = find(MI_info.MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.instReachFeatures{fidx}).binTimes==PID_params.MIsigtiming(1));
%                         sigTime_end = find(MI_info.MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.instReachFeatures{fidx}).binTimes==PID_params.MIsigtiming(2));
...
%         end
% 
%         tmpPIDout = cell(length(params.instReachFeatures),length(MI_info.MIout{animal}.spikesDay));
%         
%         for fidx = features_to_run
%             for day = 1:length(MI_info.MIout{animal}.spikesDay)
% 
%                 nonNaNtrials = find(~isnan(reachFeatures{animal}{day}.(params.instReachFeatures{fidx})));
%                 if PID_params.doShuff
%                     shuffIdxs = zeros(length(nonNaNtrials),PID_params.nShuff);
%                     for shidx = 1:PID_params.nShuff
%                         shuffIdxs(:,shidx) = randperm(length(nonNaNtrials));
...
% 
%                 pair_count = 1;
%                 for m1_unit = 1:size(MI_info.MIout{animal}.spikesDay{day}.M1.(params.instReachFeatures{fidx}).info,1)
%                     for dls_unit = 1:size(MI_info.MIout{animal}.spikesDay{day}.DLS.(params.instReachFeatures{fidx}).info,1)
%                          if sig_units{fidx,1,day}(m1_unit) && sig_units{fidx,2,day}(dls_unit)
%                             disp(['Computing... ' params.animals{animal} ' | M1 unit ' num2str(m1_unit) ' & DLS unit ' num2str(dls_unit) ' | day ' num2str(day) ' | ' params.instReachFeatures{fidx}])
% 
...
% 
%                                         Y = reachFeatures{animal}{day}.(params.instReachFeatures{fidx})(nonNaNtrials);
%                                         Y = repmat(Y,size(X1,1),1);
% 
%                                         X1 = X1(:);
%                                         X2 = X2(:);
...
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).shared = tmpPIDout{fidx}{day}.shared;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).unique_m1 = tmpPIDout{fidx}{day}.unique_m1;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).unique_dls = tmpPIDout{fidx}{day}.unique_dls;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).complementary = tmpPIDout{fidx}{day}.complementary;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).sharedSh = tmpPIDout{fidx}{day}.sharedSh;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).unique_m1Sh = tmpPIDout{fidx}{day}.unique_m1Sh;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).unique_dlsSh = tmpPIDout{fidx}{day}.unique_dlsSh;
%                             PIDout{animal}.spikes{day}.(params.instReachFeatures{fidx}).complementarySh = tmpPIDout{fidx}{day}.complementarySh;
%                         end
%                     end
%                 end
%             end
