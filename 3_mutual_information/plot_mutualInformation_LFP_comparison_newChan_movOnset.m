function [] = plot_mutualInformation_LFP_comparison_newChan_movOnset(MIout,LFP_early_late,params,clusterParams,f2plot,plot_params,newBinTimes)
%% ACTIVITY CASCADES
    ...
    %                     figure; imagesc(zscore(squeeze(LFP_early_late{animal}{early_late}.(params.lfpFeatures{n_lfp}).(params.areas{aidx})(chan,:,:)))')
        ...
        for chan = 1:length(raw_LFP.M1{1})-1
            ...
            for bins = 1:size(timeBins4LFP,1)
                tmp_LFP = [tmp_LFP nanmean(raw_LFP.M1{1}{chan+1}(:,timeBins4LFP(bins,1):timeBins4LFP(bins,2)),'all')];
            end
        ...
        for chan = 1:length(raw_LFP.M1{2})-1
            tmp_LFP = [];
            for bins = 1:size(timeBins4LFP,1)
                tmp_LFP = [tmp_LFP nanmean(raw_LFP.M1{2}{chan+1}(:,timeBins4LFP(bins,1):timeBins4LFP(bins,2)),'all')];
            end
        ...
        for chan = 1:length(raw_LFP.DLS{1})-1
            tmp_LFP = [];
            for bins = 1:size(timeBins4LFP,1)
                tmp_LFP = [tmp_LFP nanmean(raw_LFP.DLS{1}{chan+1}(:,timeBins4LFP(bins,1):timeBins4LFP(bins,2)),'all')];
            end
        ...
        for chan = 1:length(raw_LFP.DLS{2})-1
            tmp_LFP = [];
            for bins = 1:size(timeBins4LFP,1)
                tmp_LFP = [tmp_LFP nanmean(raw_LFP.DLS{2}{chan+1}(:,timeBins4LFP(bins,1):timeBins4LFP(bins,2)),'all')];
            end
    ...
    %    figure('units','normalized','outerposition',[0 0 1 1])
            figure;
                ...
                if plot_params.save == 1
                    print([plot_params.save_path '\lfp_activity_cascade_plots_movOnset.eps'],'-painters','-depsc');
                end   
...
%             disp([num2str(v) ' | ' num2str(11+i)])
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\lfp_activity_timing_movOnset.eps'],'-painters','-depsc');
        end
...
%% INFO CASCADES MAX VEL/TRAJ DIST
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\lfp_information_cascade_plots_movOnset.eps'],'-painters','-depsc');
        end   

%         save('G:\CurrBiol_revision\figures\figure_2\movOnset_comparison\MO_LFP_info.mat','m1_early_info','m1_late_info','dls_early_info','dls_late_info')        
...
%     onset_prc = 0.66;
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\lfp_information_timing_movOnset.eps'],'-painters','-depsc');
        end
