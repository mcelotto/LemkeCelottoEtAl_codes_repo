function [] = success_relevance_mutual_information_scatterplot_spikes(reach_features,MIout,LFP_early_late,params,clusterParams,save_params,newBinTimes)
...
%%% REACH FEATURES
    ...
    for fidx = 1:length(params.reachFeatures)-1 % not computed for success rate
        ...
        disp(['SPIKES | ' params.reachFeatures{fidx} ' | M1 | early: ' num2str(size(m1_early_info,1)) ' out of ' num2str(size(m1_early_info_all,1)) ' - ' num2str(100*size(m1_early_info,1)/size(m1_early_info_all,1))  ...
        ...
        disp(['SPIKES | ' params.reachFeatures{fidx} ' | DLS | early: ' num2str(size(dls_early_info,1)) ' out of ' num2str(size(dls_early_info_all,1)) ' - ' num2str(100*size(dls_early_info,1)/size(dls_early_info_all,1))  ...
...
%%% PLOT
    ...
    if save_params.save == 1
        print([save_params.save_path '\info_success_relevance_scatter_spikes.eps'],'-painters','-depsc');
    end
...
%     figure;
%     hold on;
%     scatter(all_m1_early_mean([1:8 11]),feature_tStat([1:8 11],1),30,[.5 .5 .5])
%     scatter(all_dls_early_mean([1:8 11]),feature_tStat([1:8 11],1),30,[1 .5 .5])
%     scatter(all_m1_late_mean([1:8 11]),feature_tStat([1:8 11],2),30,[0 0 0])
%     scatter(all_dls_late_mean([1:8 11]),feature_tStat([1:8 11],2),30,[1 0 0])
% 
