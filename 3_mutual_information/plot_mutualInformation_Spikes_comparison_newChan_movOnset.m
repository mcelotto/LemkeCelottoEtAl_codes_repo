function [] = plot_mutualInformation_Spikes_comparison_newChan_movOnset(MIout,spikes,params,clusterParams,f2plot,plot_params,newBinTimes)
%% ACTIVITY CASCADES
    ...
    for unit = 1:length(raw_Spikes.M1{1})-1
        ...
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes nanmean(raw_Spikes.M1{1}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
    ...
    for unit = 1:length(raw_Spikes.M1{2})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes nanmean(raw_Spikes.M1{2}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
    ...
    for unit = 1:length(raw_Spikes.DLS{1})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes nanmean(raw_Spikes.DLS{1}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
    ...
    for unit = 1:length(raw_Spikes.DLS{2})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes nanmean(raw_Spikes.DLS{2}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
    ...
    figure;
        ...
        subplot(4,2,8); hold on;
            ...
            if plot_params.save == 1
                print([plot_params.save_path '\spiking_activity_cascade_plots_movOnset.eps'],'-painters','-depsc');
            end
...
%% ACTIVITY TIMING
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\spiking_activity_timing_movOnset.eps'],'-painters','-depsc');
        end
...
%% INFORMATION CASCADES MAX VEL/TRAJ DIST
    ...
    figure;
        ...
        subplot(4,2,8); hold on;
            ...
            xlim([12 86])
        if plot_params.save == 1
            print([plot_params.save_path '\spiking_information_cascade_plots_movOnset.eps'],'-painters','-depsc');
        end   

%         save('G:\CurrBiol_revision\figures\figure_2\movOnset_comparison\MO_Spiking_info.mat','m1_early_info','m1_late_info','dls_early_info','dls_late_info')                
...
%% INFO TIMING MAX VEL/TRAJ DIST
    ...
    figure;
        ...
        if plot_params.save == 1
            ...
            print([plot_params.save_path '\spiking_information_timing_movOnset.eps'],'-painters','-depsc');
        end
