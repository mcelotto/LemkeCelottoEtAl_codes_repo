function [] = plot_mutualInformation_Spikes_comparison_newChan(MIout,spikes,params,clusterParams,f2plot,plot_params,newBinTimes)
%% ACTIVITY CASCADES
    ...
    for animal = 1:numel(params.animals)
        for aidx = 1:length(params.areas)
            ...
            for day = 1:params.num_earlylate_days{animal}
                for unit = 1:size(MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.reachFeatures{9}).info,1)
    ...
    m1_early_info_spikes_z = [];
    for unit = 1:length(raw_Spikes.M1{1})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes mean(raw_Spikes.M1{1}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
        m1_early_info_spikes_z = [m1_early_info_spikes_z; smoothdata(zscore(tmp_spikes),'gaussian',5)];
    end
    
    m1_late_info_spikes_z = [];
    for unit = 1:length(raw_Spikes.M1{2})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes mean(raw_Spikes.M1{2}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
        m1_late_info_spikes_z = [m1_late_info_spikes_z; smoothdata(zscore(tmp_spikes),'gaussian',5)];
    end
    
    dls_early_info_spikes_z = [];
    for unit = 1:length(raw_Spikes.DLS{1})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes mean(raw_Spikes.DLS{1}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
        dls_early_info_spikes_z = [dls_early_info_spikes_z; smoothdata(zscore(tmp_spikes),'gaussian',5)];
    end
    
    dls_late_info_spikes_z = [];
    for unit = 1:length(raw_Spikes.DLS{2})-1
        tmp_spikes = [];
        for bins = 1:size(timeBins4spikes,1)
            tmp_spikes = [tmp_spikes mean(raw_Spikes.DLS{2}{unit+1}(:,timeBins4spikes(bins,1):timeBins4spikes(bins,2)),'all')];
        end
        dls_late_info_spikes_z = [dls_late_info_spikes_z; smoothdata(zscore(tmp_spikes),'gaussian',5)];
    end
    
    % figure('units','normalized','outerposition',[0 0 1 1])
    figure;
        subplot(4,2,1); hold on;
            [~, i_info] = max(m1_early_info_spikes_z');
            [~, i_info] = sort(i_info);
            imagesc(m1_early_info_spikes_z(i_info,:),[-3 5]);
            plot([61.5 61.5],[1 length(i_info)],'k','LineWidth',1,'LineStyle','--')
            xlim([12 86])
            ylim([1 length(i_info)])
            title(num2str(length(i_info)))
        subplot(4,2,2); hold on;
            [~, i_info] = max(m1_late_info_spikes_z');
            [~, i_info] = sort(i_info);
            imagesc(m1_late_info_spikes_z(i_info,:),[-3 5]);
            plot([61.5 61.5],[1 length(i_info)],'k','LineWidth',1,'LineStyle','--')
            xlim([12 86])
            ylim([1 length(i_info)])
            title(num2str(length(i_info)))
        subplot(4,2,3); hold on;
            [~, i_info] = max(dls_early_info_spikes_z');
            [~, i_info] = sort(i_info);
            imagesc(dls_early_info_spikes_z(i_info,:),[-3 5]);
            plot([61.5 61.5],[1 length(i_info)],'k','LineWidth',1,'LineStyle','--')
            xlim([12 86])
            ylim([1 length(i_info)])
            title(num2str(length(i_info)))
        subplot(4,2,4); hold on;
            [~, i_info] = max(dls_late_info_spikes_z');
            [~, i_info] = sort(i_info);
            imagesc(dls_late_info_spikes_z(i_info,:),[-3 5]);
            plot([61.5 61.5],[1 length(i_info)],'k','LineWidth',1,'LineStyle','--')
            xlim([12 86])
            ylim([1 length(i_info)])
            title(num2str(length(i_info)))
        subplot(4,2,5); hold on;
            plot(nanmean(abs(m1_early_info_spikes_z))+nanstd(abs(m1_early_info_spikes_z))/sqrt(size(abs(m1_early_info_spikes_z),1)),'k')
            plot(nanmean(abs(m1_early_info_spikes_z))-nanstd(abs(m1_early_info_spikes_z))/sqrt(size(abs(m1_early_info_spikes_z),1)),'k')
              ylim([.3 1.4])
            xlim([12 86])
        subplot(4,2,6); hold on;
            plot(nanmean(abs(m1_late_info_spikes_z))+nanstd(abs(m1_late_info_spikes_z))/sqrt(size(abs(m1_late_info_spikes_z),1)),'k')
            plot(nanmean(abs(m1_late_info_spikes_z))-nanstd(abs(m1_late_info_spikes_z))/sqrt(size(abs(m1_late_info_spikes_z),1)),'k')
            ylim([.3 1.4])
            xlim([12 86])            
        subplot(4,2,7); hold on;
             plot(nanmean(abs(dls_early_info_spikes_z))+nanstd(abs(dls_early_info_spikes_z))/sqrt(size(abs(dls_early_info_spikes_z),1)),'r')
            plot(nanmean(abs(dls_early_info_spikes_z))-nanstd(abs(dls_early_info_spikes_z))/sqrt(size(abs(dls_early_info_spikes_z),1)),'r')
              ylim([.3 1.4])
            xlim([12 86])            
        subplot(4,2,8); hold on;
            plot(nanmean(abs(dls_late_info_spikes_z))+nanstd(abs(dls_late_info_spikes_z))/sqrt(size(abs(dls_late_info_spikes_z),1)),'r')
            plot(nanmean(abs(dls_late_info_spikes_z))-nanstd(abs(dls_late_info_spikes_z))/sqrt(size(abs(dls_late_info_spikes_z),1)),'r')
              ylim([.3 1.4])
            xlim([12 86])            
            if plot_params.save == 1
                print([plot_params.save_path '\spiking_activity_cascade_plots.eps'],'-painters','-depsc');
            end
...
%     onset_prc = 0.66;
    ...
    m1_early_onset = [];
    m1_early_peak = [];
    for unit = 1:size(m1_early_info_spikes_z,1)
        baseSD = std(m1_early_info_spikes_z(unit,baseline_period));
        baseMean = mean(m1_early_info_spikes_z(unit,baseline_period));
        [v, i] = max(m1_early_info_spikes_z(unit,12:86)); % -1s to +500ms
        if v>baseMean+(baseSD*sd_thresh) & i~=length(12:86)
            m1_early_onset = [m1_early_onset min(find(m1_early_info_spikes_z(unit,:)>v*onset_prc))];
    ...
    m1_late_onset = [];
    m1_late_peak = [];
    for unit = 1:size(m1_late_info_spikes_z,1)
        baseSD = std(m1_late_info_spikes_z(unit,baseline_period));
        baseMean = mean(m1_late_info_spikes_z(unit,baseline_period));
        [v, i] = max(m1_late_info_spikes_z(unit,12:86)); % -1s to +500ms
        if v>baseMean+(baseSD*sd_thresh) & i~=length(12:86)
            m1_late_onset = [m1_late_onset min(find(m1_late_info_spikes_z(unit,:)>v*onset_prc))];
    ...
    dls_early_onset = [];
    dls_early_peak = [];
    for unit = 1:size(dls_early_info_spikes_z,1)
        baseSD = std(dls_early_info_spikes_z(unit,baseline_period));
        baseMean = mean(dls_early_info_spikes_z(unit,baseline_period));
        [v, i] = max(dls_early_info_spikes_z(unit,12:86)); % -1s to +500ms
        if v>baseMean+(baseSD*sd_thresh) & i~=length(12:86)
            dls_early_onset = [dls_early_onset min(find(dls_early_info_spikes_z(unit,:)>v*onset_prc))];
    ...
    dls_late_onset = [];
    dls_late_peak = [];
    for unit = 1:size(dls_late_info_spikes_z,1)
        baseSD = std(dls_late_info_spikes_z(unit,baseline_period));
        baseMean = mean(dls_late_info_spikes_z(unit,baseline_period));
        [v, i] = max(dls_late_info_spikes_z(unit,12:86)); % -1s to +500ms
        if v>baseMean+(baseSD*sd_thresh) & i~=length(12:86)
            dls_late_onset = [dls_late_onset min(find(dls_late_info_spikes_z(unit,:)>v*onset_prc))];
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\spiking_activity_timing.eps'],'-painters','-depsc');
        end

%% INFORMATION CASCADES MAX VEL/TRAJ DIST
            ...
            for aidx = 1:length(params.areas)
                ...
                for day = 1:params.num_earlylate_days{animal}
                    for unit = 1:size(MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.reachFeatures{fidx}).info,1)
                        ...
                        info_mean_all.(params.areas{aidx}){1,feature} = [info_mean_all.(params.areas{aidx}){1,feature}; infQuant];                        
                        infQuant(~tmp_sig) = 0;
                        info_mean_sig.(params.areas{aidx}){1,feature} = [info_mean_sig.(params.areas{aidx}){1,feature}; infQuant];
                    end
                end
                % late
                for day = length(MIout{animal}.spikesDay)-params.num_earlylate_days{animal}+1:length(MIout{animal}.spikesDay)
                    for unit = 1:size(MIout{animal}.spikesDay{day}.(params.areas{aidx}).(params.reachFeatures{fidx}).info,1)
                        ...
                        tmp_sig = clusterStat_v2(infQuant,infQuantSh,clusterParams(1),clusterParams(2));
                        infQuant = infQuant-mean(squeeze(infQuantSh),2)';
                        info_mean_all.(params.areas{aidx}){2,feature} = [info_mean_all.(params.areas{aidx}){2,feature}; infQuant];
                        infQuant(~tmp_sig) = 0;
                        info_mean_sig.(params.areas{aidx}){2,feature} = [info_mean_sig.(params.areas{aidx}){2,feature}; infQuant];
                    end
                end
            end
        end
    ...
    figure;
        subplot(4,2,1); hold on;
            ...
            imagesc([m1_early_info(i_early_info,:)],[0 .1]);
        ...
        subplot(4,2,2); hold on;
            ...
            imagesc([m1_late_info(i_late_info,:)],[0 .1]);
        ...
        subplot(4,2,3); hold on;
            ...
            [~, i_early_info] = sort(i_early_info);
            imagesc([dls_early_info(i_early_info,:)],[0 .1]);
            plot([1 98],[length(i_early_info)],'r','LineWidth',1)
            xlim([12 86])
            ylim([1 length(i_early_info)])       
            title(num2str(length(i_early_info)))                                                
        subplot(4,2,4); hold on;
            ...
            [~, i_late_info] = sort(i_late_info);
            imagesc([dls_late_info(i_late_info,:)],[0 .1]);
            plot([1 98],[length(i_late_info)],'r','LineWidth',1)
            xlim([12 86])
            ylim([1 length(i_late_info)])      
            title(num2str(length(i_late_info)))                                                
        ...
        subplot(4,2,8); hold on;
            ...
            xlim([12 86])
        if plot_params.save == 1
            print([plot_params.save_path '\spiking_information_cascade_plots.eps'],'-painters','-depsc');
        end   

%         save('G:\CurrBiol_revision\figures\figure_2\movOnset_comparison\PT_spikes_info.mat','m1_early_info','m1_late_info','dls_early_info','dls_late_info')
...
%% INFO TIMING MAX VEL/TRAJ DIST
    ...
    %%% FEATURE 1
        ...
        for unit = 1:size(m1_early_info,1)
            baseSD = std(m1_early_info(unit,baseline_period));
            baseMean = mean(m1_early_info(unit,baseline_period));
        ...
        for unit = 1:size(m1_late_info,1)
            baseSD = std(m1_late_info(unit,baseline_period));
            baseMean = mean(m1_late_info(unit,baseline_period));
        ...
        for unit = 1:size(dls_early_info,1)
            baseSD = std(dls_early_info(unit,baseline_period));
            baseMean = mean(dls_early_info(unit,baseline_period));
        ...
        for unit = 1:size(dls_late_info,1)
            baseSD = std(dls_late_info(unit,baseline_period));
            baseMean = mean(dls_late_info(unit,baseline_period));
    ...
    %%% FEATURE 2
        ...
        for unit = 1:size(m1_early_info2,1)
            baseSD = std(m1_early_info2(unit,baseline_period));
            baseMean = mean(m1_early_info2(unit,baseline_period));
        ...
        for unit = 1:size(m1_late_info2,1)
            baseSD = std(m1_late_info2(unit,baseline_period));
            baseMean = mean(m1_late_info2(unit,baseline_period));
        ...
        for unit = 1:size(dls_early_info2,1)
            baseSD = std(dls_early_info2(unit,baseline_period));
            baseMean = mean(dls_early_info2(unit,baseline_period));
        ...
        for unit = 1:size(dls_late_info2,1)
            baseSD = std(dls_late_info2(unit,baseline_period));
            baseMean = mean(dls_late_info2(unit,baseline_period));
    ...
    figure;
        ...
        if plot_params.save == 1
            print([plot_params.save_path '\spiking_information_timing.eps'],'-painters','-depsc');
        end
